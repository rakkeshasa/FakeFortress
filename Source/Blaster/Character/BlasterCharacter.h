...
private:	
	...
	// 서버에서 다른 클라로 복제될 무기(팝업 창이 다른 클라한테 보이기 위해)
	UPROPERTY(ReplicatedUsing = OnRep_OverlappingWeapon)
	class AWeapon* OverlappingWeapon;

	// 어떤 무기가 복제될 때마다 자동으로 호출될 함수(이런 종류는 앞에 OnRep_가 붙는다)
	// Rep_notify 함수는 복제될 변수만 입력변수로 가질수 있다.
	UFUNCTION()
	...
	*/
	...
	// RPC로 만들기 위해 Server 속성 추가
	// 신뢰가 보장된 RPC는 실행을 보장한다.(패킷 전송에 실패하면 다시 RPC를 다시 보낸다)
	// 비신뢰일 시 클라이언트에서 서버로 정보를 보낼 때 RPC가 중단될 가능성이 있다.
	UFUNCTION(Server, Reliable)
...
public:
	// 복제할 무기가 항상 복제되는것이 아니라 충돌한 순간만 복제한다.
